...
struct matrix * make_rotX(double theta) {
  double degree = theta * M_PI / 180;
  struct matrix * temp = new_matrix(4,4);
  ident(temp);
  temp->m[1][1] = cos(degree);
  temp->m[1][2] = -sin(degree);
  temp->m[2][1] = sin(degree);
  temp->m[2][2] = cos(degree);
  return temp;
}
...
struct matrix * make_rotY(double theta) {
  ...
  struct matrix * temp = new_matrix(4,4);
  ident(temp);
  temp->m[0][0] = cos(degree);
  temp->m[0][2] = sin(degree);
  temp->m[2][0] = -sin(degree);
  temp->m[2][2] = cos(degree);
  return temp;
}
...
struct matrix * make_rotZ(double theta) {
  ...
  ident(temp);
  temp->m[0][0] = cos(degree);
  temp->m[0][1] = -sin(degree);
  temp->m[1][0] = sin(degree);
  temp->m[1][1] = cos(degree);
  return temp;
}
